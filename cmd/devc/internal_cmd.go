		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, df := range m.Dotfiles {
				...
				if _, err := os.Stat(staging); err != nil {
					ui.PrintWarn("Skip", fmt.Sprintf("%s (not found in staging)", rel))
					continue
				}

				// Ensure parent directory
				if err := os.MkdirAll(filepath.Dir(target), 0o755); err != nil {
					ui.PrintWarn("Skip", fmt.Sprintf("%s: %v", rel, err))
					continue
				}
				...
				if err := os.Symlink(staging, target); err != nil {
					...
					ui.PrintDone("Linked", fmt.Sprintf("%s → %s", target, staging))
				}
			}
			return nil
		},
...
func sendDaemonRequest(req daemon.Request) (*daemon.Response, error) {
	conn, err := net.Dial("unix", daemon.DevcSockPath)
	if err != nil {
...
Examples:
  ...
  devc port 9090:3000     # forward container:3000 → host:9090`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			resp, err := sendDaemonRequest(daemon.Request{
				Type: "port",
				Port: args[0],
			})
			if err != nil {
			...
			ui.PrintDone("Port forwarded", resp.Message)
			return nil
		},
	}
}
...
func newRebuildInternalCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ui.PrintDone("Rebuild requested", resp.Message)
			return nil
		},
	}
}
