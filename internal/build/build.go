...
func BuildFeatureImage(ctx context.Context, ws config.Workspace, cfg *config.DevcontainerConfig,
	...
	if _, err := cli.ImageInspect(ctx, imageTag); err == nil {
		ui.PrintDone("Using cached image", imageTag)
		return imageTag, nil
	}

	if len(allFeatures) == 0 {
		// No features — just pull and tag the base image
		if err := ui.RunWithSpinner("Pulling base image", baseImage, func() error {
			reader, err := cli.ImagePull(ctx, baseImage, image.PullOptions{})
			if err != nil {
				return fmt.Errorf("pull image: %w", err)
			}
			if err := ui.DrainDockerOutput(reader); err != nil {
				_ = reader.Close()
				return fmt.Errorf("pull image: %w", err)
			}
			_ = reader.Close()
		...
		ui.PrintDone("Pulled base image", baseImage)
	...
	if err := ui.RunWithSpinner("Building image", imageTag, func() error {
		...
		if err := ui.DrainDockerOutput(resp.Body); err != nil {
			return fmt.Errorf("image build: %w", err)
		}
		return nil
	}); err != nil {
...
func BuildUserDockerfile(ctx context.Context, ws config.Workspace, cfg *config.DevcontainerConfig) (string, error) {
	cli, err := docker.GetClient()
	if err != nil {
		return "", fmt.Errorf("docker client: %w", err)
	}
	...
	if err := ui.RunWithSpinner("Building Dockerfile", dockerfilePath, func() error {
	...
	ui.PrintDone("Built Dockerfile", dockerfilePath)
